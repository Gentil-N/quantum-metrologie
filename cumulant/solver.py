...
#from corr_sys_ada_order_5 import *
...
#x_init_ada_order_2 = corr_get_init_vec_ada_order_2(state)
...
#solution_order_2 = solve_ivp(corr_system_ada_order_2, [0.0, 0.001], x_init_ada_order_2)
...
#for i in range(len(solution_order_2.y)):
#    ax.plot(solution_order_2.t, np.real(solution_order_2.y[i]), label=str(i))
...
fig, ax = pyplot.subplots(1, 1)
fig.set_size_inches(18.5, 10.5)
ax.plot(solution_order_3.t, np.real(solution_order_3.y[27]), label="<n> 3nd re", color="green", linestyle="--")
ax.plot(solution_order_3.t, np.real(solution_order_3.y[25]), label="<Sz> 3nd re", color="green")
ax.plot(solution_order_3.t, np.imag(solution_order_3.y[27]), label="<n> 3nd imag", color="orange", linestyle="--")
ax.plot(solution_order_3.t, np.imag(solution_order_3.y[25]), label="<Sz> 3nd imag", color="orange")
ax.legend()
ax.set_xlabel('time')
ax.set_ylabel('correlations')
#
...
ax.plot(g1_solution_order_3.t, np.abs(g1_solution_order_3.y[0]) / np.abs(g1_solution_order_3.y[42][0]), label="<ad(t)ac> 3rd steady-state", color="red")
