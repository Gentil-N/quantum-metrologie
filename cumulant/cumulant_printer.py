...
def g1_cumulant_printer(op_init, op, order):
    ...
    print("File creation...", end="", flush=True)
    python_func = convert_corr_equ_list_to_python_function(comp_corr_equ_list, order, op, "g1", True)
    python_vec_init = get_corr_python_vec_init_from_other_corr(comp_corr_equ_list, comp_init_corr_equ_list, init_dict, "g1", order, op)
    file = open("g1_sys_" + op.get_simple_str() + "_order_" + str(order) + ".py", 'w')
    file.write(python_func + "\n\n" + python_vec_init)
    file.close()
    print("Done")
...
g1_cumulant_printer(OP_Ad, OP_Ad * OP_Ac, 4)