...
\end{itemize}
The experiement is configured as the input electric fields being linearly and parallely polarized: so we consider $\vv{E}_{cav}$ and $\vv{E}_{LO}$ as such. This simplifies the equation by only taking into account the amplitude of the fields instdead of their vector components. Also, as the intensity calculation involve a scalar product of the electric vector fields, the linear polarization allows to maximise the light power when mixing two E-fields.
...
\begin{align}
\begin{split}
E_{cav}(t) &=\frac{{E}_{cav}}{2} e^{i\omega_{c}t} + \frac{{E}_{cav}}{2} e^{-i\omega_{c}t} \\
E_{LO}(t) &= \frac{{E}_{LO}}{2} e^{i\omega_{LO}t} + \frac{{E}_{LO}}{2} e^{-i\omega_{LO}t}
\end{split}
\end{align}
that is to say, we consider $E_{cav}(t)$ as perfectly coherent (i.e monochromatic) and we omit the phase difference, the whole for the sake of simplicity as the excpected result effects would be similar in case of phase unmatch and quasi-monochromatic cavity field. Note that $E_{cav/LO}$ without the time dependence $t$ is the amplitude of the respective field.

Now appling the exact formula $I_{total}(t) = E_{total}(t)^2$, one gets four different terms\footnote{See annexe ??}:
\begin{align}
...
\begin{itemize}
	...
	\item The amplified power noise: when amplifying the photocurrent through the circuit, the photocurrent noise is also amplified and is linearly dependent to the mean intensity light for Poissonian distribution of the photons (i.e for classical light)\footnote{See Fox ??}. In other words, more powerful is the incoming beam, more important is the variance (i.e the shift to the mean) of the photocurrent after amplification.
\end{itemize} 
The first two types of noises are uncorellated and just add together while the amplified power noise is dependent of both the quantum and electrical noise as it amplifies them. 

Thus, the idea behind balanced detection is to cancel the classical noise which can randomly appear on both diodes of the detector at the same time. Indeed, if any correlated variation of the signal apperas in both emited photocurrent, the substraction \textit{erases} this correlated variation (i.e noise) in a sense that the resulting signal then owns the quantum noise, the uncorrorelated photocurrent noise (which may be amplified) as well as the electric noise (which is a white noise, hence uncorrelated). Nevertheless, the remaining noise may be considered as very small for our purpose and the detector is made in order to reduce the electrical and amplified power noise in the bandwidth of interest. This reduction of noise by electronic design is the subject of the next section.
...
Those two capacitors add with an internal capacitance of the op-amp for which the value may be found in its associated data sheet. Here, we deal with a THS4021 from Texas Instrument for the op-amp and two S5973 photodiodes from Hamamatsu\footnote{Those two diodes have a high speed response of 1GHz: far more than the expected freqency of 43MHz.}. In this configuration, those three capacitors are in parallele and the total capacitance reads:
\begin{equation}
...
where the deep of $-170$dBm clearly corresponds to the $43$MHz resonance. Thus, modifying the inductor to $0.82\mu F$ should move the deep to the correct position of $43$Mhz. Moreover, in the worst case of misunderstanding this misalignment with the theoretical resonances, the inductor would change values from several hundreds of micro-Farads, in order to reach a theoretical maximum of $60$MHz: for example, taking $L=0.5\mu F$ would give approximately $60$Mhz of resonance for the deep and considering a shift to the left, we should hit the targeted deep of $43$MHz. In that way, we commanded several inductors of values ranging $100nF$ to $2\mu F$ and replace the orginal inductor by removing and making a new soldered joint.
...
The next section will focus on the parametrization/configuration of the detector which involve, among others, saturation test, amplification noise test and technical procedures in order to make sure that the experiment works in the parameter's range of the detector.
\end{document}