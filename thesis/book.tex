...
\end{equation} In other words, it just makes the link between the enhancment factor of the cavity field and the constructive-destructive interference phase pattern. In order to derive it, we shall use the circulating field approach \footnote{A. E. Siegman, section 11.3, \textit{Lasers}}, represented in Fig (add figure!):
\begin{itemize}
...
\begin{equation}
\sum_{j=1}^{N} \hat{\sigma}_i^j \rightarrow \frac{2}{\hbar} \hat{S}_j ATTENTION !!! PEUT ETRE PAS BON
\end{equation}
