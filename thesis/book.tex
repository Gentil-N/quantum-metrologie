...
\begin{equation}
\overline{E}(x, t) \stackrel{\text{def}}{=} \overline{E}e^{i\omega t + \phi}
\end{equation}
where $\overline{E}$ is the complex ampltiude (constant), $\omega$ the frequency of this perfectly coherent field and $\phi$ the phase at the origin. The act of \textit{phase modulating} comes while adding $h\sin(\Omega t)$ to the phase $\phi$, where $h$ is the modulation index and $\Omega$ the modulation frequency. Thus:
\begin{equation}
\overline{E}(x, t) \rightarrow \overline{E}_{mod}(x, t) \stackrel{\text{def}}{=} \overline{E}e^{i\omega t + h\sin(\Omega t) + \phi}
\end{equation}
...
we find:
\begin{equation}
\overline{E}(x, t) = \overline{E}e^{i\omega t + \phi} \left( J_0(h) + \sum_{n=1}^{+\infty} J_n(h)e^{in\Omega t} + \sum_{n=1}^{+\infty} (-1)^n J_n(h)e^{-in\Omega t} \right)
\end{equation}
...
\begin{equation}
\overline{E}(x, t) \simeq \overline{E}e^{i\omega t + \phi} \left( J_0(h) + J_1(h)e^{i\Omega t} - J_1(h)e^{-i\Omega t} \right)
\end{equation}
...
The red curve is a \textit{bessel} fit accross all the data points. Indeed, as the detector (not the balanced detector) receives separately the first sideband, the carrier and the second sideband, the intensities read as:
\begin{align}
I_m &\simeq \vert J_0(h)\overline{E}e^{i\omega t + \phi} \vert ^2\\
I_{s,left} &\simeq \vert -J_1(h)\overline{E}e^{i\omega t + \phi - \Omega t} \vert ^2\\
I_{s,right} &\simeq \vert J_1(h)\overline{E}e^{i\omega t + \phi + \Omega t} \vert ^2
\end{align}
and averaging over a couple of periods:
\begin{align}
\left\langle I_m \right\rangle &\simeq 2J_1(h)^2\\
\left\langle I_{s,left} \right\rangle = \left\langle I_{s,right} \right\rangle &\simeq 2J_0(h)^2
\end{align}
By definition of the ratio $r$, we get:
\begin{equation}
r = \left\langle \frac{I_s}{I_m} \right\rangle = \left( \frac{J_1(h)}{J_0(h)} \right)^2
\end{equation}
The variable $h$ is then determined numerically. As we can see, the red curve including every experimental data point is not a good approximation: we suspect a saturation of the modulation device above a certain amplitude where this \textit{bessel formulation} is not valid anymore. Therefore, the green curve is a bessel fit over the first 55\% of amplitude which gives a better approximation. However, even without physical argument, it is easier to trust the exponenital fit and that is why the $\alpha$ ratio is calulated upon it.
