...
\begin{align}
E_{n, \pm} &= \hbar\Delta\omega\,(n-\frac{1}{2}) \pm \frac{\hbar}{2}\sqrt{4g^2n + \Delta\omega ^2}\\
\ket{n, +} &= \cos\left(\frac{\theta_n}{2}\right) \ket{\uparrow, n - 1}  + \sin\left(\frac{\theta_n}{2}\right) \ket{\downarrow, n}\\
\ket{n, -} &= \cos\left(\frac{\theta_n}{2}\right) \ket{\downarrow, n}  - \cos\left(\frac{\theta_n}{2}\right) \ket{\uparrow, n - 1}
\end{align}
with:
\begin{equation}
...
\begin{equation}
\ket{\psi(0)} = \ket{\uparrow, n = 0} = \cos\left(\frac{\theta_{1}}{2}\right) \ket{1, +} - \sin\left(\frac{\theta_{1}}{2}\right) \ket{1, -}
\end{equation}
...
Numerically, if one puts a small detuning (i.e $\omega_a$ is still about the same order of magnitude of $omega_c$), one can numerically sketch on Fig ?? where we see that the energy transfer is progressively discarded as we increase the detuning: the probabilities of finding an inversed state decreases. 
