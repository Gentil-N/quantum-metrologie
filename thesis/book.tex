...
\begin{equation}
\textrm{Gaussian fit}(\tau, A, \sigma) = \frac{A}{2\pi\sigma} e^{-\frac{\tau^2}{2\sigma^2}} + 0.1
\end{equation}
...
\end{equation} 
then a numerical integration gives $\tau_c \simeq 71$µs. In other words, for the time interval where the superradiant laser exhibits a stationary process (i.e between 0.5ms and 3ms on the time interval), the frequency emitted is \textit{fairly} coherent for 40µs ($\vert g^1(\tau) \vert > 0.6$), coherent for 60µs\footnote{Due to the global offset of the curves in \ref{fig:sp-stat} along the y-axis, we can predict a slight reduction of the coherence time for the \textit{real} curve.} and mostly incoherent after 80µs.
