...
\begin{itemize}
	...
	\end{equation}
	\item A waveplate: its working principle is described in Annexe ??. Here we use a \textit{lambda-half} waveplate (abbreviated $\lambda/2$-waveplate): for a linearly polarized electric field, this device simply rotate the polarization's direction of the electric field.  
\end{itemize}
...
The waveplate rotate the electric field. The polarizer project the electric field. If the E-field is parallel to the polarizer axis, the output amplitude is the same as the input one. If the E-field is perpendicular to the polarizer axis, the ouput amplitude is null. Every E-field polarisation's direction in between shall give a reduced amplitude compared 
to the maximum amplitude: because of the scalar operation, the amplitude relation is not linear but \textit{cosine} dependent, hence very sensitive for almost perpendicular E-fields.
